...
import { GEMINI_API_KEY, GEMINI_MODEL, BEARISH_THRESHOLD, BULLISH_THRESHOLD } from "@/lib/config";
...
// ─────────────────────────────────────────────────────────────────────────────
// THE PERFECT SYSTEM PROMPT (ADAPTED FOR GEMINI)
// ─────────────────────────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `
...
export async function getPrediction(
  ...
  try {
    ...
    const prediction = JSON.parse(cleaned) as Prediction;
    ...
    logger.error(`[Predictor] Error or failed to parse Gemini response: ${err.message}`);
    return {
      signal: "NEUTRAL", confidence: 0, action: "HOLD", urgency: "LOW",
      reasoning: "Failed to parse AI response or model error. Defaulting to HOLD for safety.",
    };
  }
}
