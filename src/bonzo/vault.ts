...
import {
  HEDERA_RPC_URL, HEDERA_PRIVATE_KEY, HEDERA_ACCOUNT_ID,
...
export async function getVaultPosition(evmAddress: string): Promise<VaultPosition> {
  ...
  const shares: bigint        = await vault.balanceOf(evmAddress);
...
export async function getUnderlyingBalance(evmAddress: string): Promise<bigint> {
  const token = new ethers.Contract(VAULT_UNDERLYING_TOKEN, ERC20_ABI, getProvider());
  return token.balanceOf(evmAddress);
}

export async function depositToVault(amount: bigint, receiver: string): Promise<ethers.TransactionReceipt> {
  ...
  logger.info(`[Bonzo] Approving vault ${BONZO_VAULT_ADDRESS} to spend ${amount.toString()} underlying tokens ...`);
...
export async function redeemAllShares(owner: string): Promise<ethers.TransactionReceipt> {
  const signer = getSigner();
  const vault  = new ethers.Contract(BONZO_VAULT_ADDRESS, VAULT_ABI, signer);
  const shares: bigint = await vault.balanceOf(owner);
...
export async function redeemPercent(percent: number, owner: string): Promise<ethers.TransactionReceipt> {
  if (percent <= 0 || percent > 100) throw new Error("percent must be 1-100");
  const signer = getSigner();
  const vault  = new ethers.Contract(BONZO_VAULT_ADDRESS, VAULT_ABI, signer);
  const totalShares: bigint    = await vault.balanceOf(owner);
  ...
  const tx      = await vault.redeem(sharesToRedeem, owner, owner);
  const receipt = await tx.wait();
